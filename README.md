...
** Концевенко Алексей (@kirieshki24) — Тимлид проекта**
* Создание GitHub-организации, настройка репозиториев проекта и подключение исходного кода как `submodule` к репозиторию с документацией.
* Разработка базовой архитектуры приложения и настройка графа многооконной навигации с использованием библиотеки `Decompose`.
* Настройка DevOps: реализация непрерывной сборки (CI/CD) через **GitHub Actions** с генерацией APK-артефактов.
* Настройка облачного тестирования интерфейса через интеграцию **Firebase Test Lab** в пайплайн GitHub Actions.

** Кислов Илья (@Grimb000) — Разработчик (Data & UI)**
* Проектирование схемы базы данных **Room (SQLite)**: создание сущностей (`ProfileEntity`, `HistoryEntryEntity`) и DAO для локального хранения данных.
* Верстка экранов приложения с использованием декларативного UI-фреймворка **Jetpack Compose** (Material Design 3).
* Реализация логики контроля лимитов (всплывающие Dialog-окна при превышении).
* Внедрение тач-интерфейса: механика **Swipe-to-Dismiss** для удаления элементов списка.

** Степук Анастасия (@yuyuu-8) — Разработчик (Network & QA)**
* Интеграция внешнего **XML API** Центрального Банка РФ для получения актуальных курсов валют.
* Разработка логики XML-парсера и математики динамической конвертации валют (кросс-курсы).
* Обеспечение потокобезопасности: перенос работы с БД и сетевых запросов в фоновые потоки с помощью **Kotlin Coroutines** (`Dispatchers.IO`).
* Написание автоматических Unit-тестов и UI-тестов (Compose UI Test / Espresso) для проверки сценариев использования приложения.
